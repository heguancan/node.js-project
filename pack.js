...
function _connent(callback) {
     var url='mongodb://127.0.0.1:27017/studentms';
    //连接数据库
...
// 查找数据
exports.findpage = function (collectionName, json,row,rows,callback) {
    var result = [];    //结果数组   
    //连接数据库，连接之后查找所有
    _connent(function (err, db) {
        var cursor = db.collection(collectionName).find(json).sort().limit(row).skip(rows);
        cursor.each(function (err, doc) {
            if (err) {
                callback(err, null);
                db.close(); //关闭数据库
